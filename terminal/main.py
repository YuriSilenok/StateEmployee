...
def replaceIncorrectSymbol(line):
        ...
        for indInst in range(len(instances)):
            ...
            for indRow, row in table.iterrows():
                    ...
                    if text != "":
                        ...
                        for indLine in range(len(lines)):
                            ...
                            draw.text(
                                ...
                                fill=color, 
                                font=font)
                folder = replaceIncorrectSymbol(row[imgXML.attrib['folder']])
                ...
                if not os.path.exists(path): 
                    os.makedirs(path)
                name = os.path.join(path , replaceIncorrectSymbol(row[imgXML.attrib['file']]) + '.jpg')
                print(name)
                img.save(name)
